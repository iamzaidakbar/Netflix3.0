...
const useUserProfile = () => {
  ...
  const deleteProfile = async (profileKey) => {
    ...
    const profileRef = ref(database, `profiles/${userID}/userProfiles/${profileKey}`);
  ...
  const addProfile = async (profileDetails) => {
    const userID = auth.currentUser.uid;
    const newProfileKey = push(ref(database, `profiles/${userID}/userProfiles`)).key;

    try {
      await set(ref(database, `profiles/${userID}/userProfiles/${newProfileKey}`), {
        ...profileDetails,
  ...
  useEffect(() => {
    ...
    }, [componentChanged]); // Include componentChanged in the dependency array

    return () => unsubscribe(); // Cleanup the subscription when the component unmounts
  }, [componentChanged]);

  return { currentProfileData, allProfilesData, loading, switchProfile, deleteProfile, addProfile };
};
